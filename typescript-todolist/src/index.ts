...
import { Command, CommandDeleteTodos, CommandNewTodos, CommandPrintTodos } from "./Commands";
import { waitForInput } from "./Input";
import Todo from "./Todo";
import { Action, AppState, Priority } from "./type";

const commands: Command[] = [new CommandPrintTodos(), new CommandNewTodos(), new CommandDeleteTodos()];
...
async function main() {
  ...
  while (true) {
    ...
    console.log();
    const key = await waitForInput(chalk`{yellow.bold input command:} `);
    console.clear();
